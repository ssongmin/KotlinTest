        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            FirebaseFirestore.getInstance().collection("profileImages").document(alarmDTOList[position].uid!!).get().addOnCompleteListener { task ->
                ...
                if(task.isSuccessful){
                    ...
                    Glide.with(view.context).load(url).apply(RequestOptions().circleCrop()).into(view.commentviewitem_imageview)
                }
            }

            when(alarmDTOList[position].kind){
                 0->{
                     var str_0 = alarmDTOList[position].userId + "좋아요를 눌렀습니다"
                     view.commentviewitem_textview_profile.text = str_0
                 }

                1->{
                    var str_0 = alarmDTOList[position].userId + "댓글을 작성했습니다. "+alarmDTOList[position].message
                    view.commentviewitem_textview_profile.text = str_0
                }


                2->{
                    var str_0 = alarmDTOList[position].userId + "팔로우를 했습니다"
                    view.commentviewitem_textview_profile.text = str_0
                }
            }
